...
\begin{minipage}{0.5\hsize}
 \begin{center}
  \fig[width=0.82\hsize]{./fig/discussion/ZINC02599970.eps}
  \caption{ホルモテロール}
...
\begin{minipage}{0.5\hsize}
 \begin{center}
  \fig[width=0.70\hsize]{./fig/discussion/ZINC03782818.eps}
  \caption{カンデサルタン}
...
%しかし、本研究で評価に利用したデータセットであるDUD-E\cite{Mysinger2012}やDEKOIS\cite{Bauer2013}、MUV\cite{Rohrer2009}などの
%ドッキングシミュレーション手法を比較するために作られたデータセットは、1つのターゲットに対して数千個～数万個程度の化合物しか登録されておらず、
...
EF (0.1\%)などの小さな割合におけるEFの評価（表\ref{table:usecase_accuracy}）を用い、実際のバーチャルスクリーニングでの提案手法
の利用例を示す。
